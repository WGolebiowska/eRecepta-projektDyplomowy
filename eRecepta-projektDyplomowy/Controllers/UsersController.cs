...
namespace eRecepta_projektDyplomowy.Controllers
{
    ...
    public class UsersController : ControllerBase
    {
        ...
        public async Task<IActionResult> Put(string id, UpdateUserModel userModel)
        {
            ...
            try
            {
                await _umService.UpdateUserAsync(user, userModel.Role);
            }
            catch (DbUpdateConcurrencyException ex)
            {
                ...
                {
                    ModelState.AddModelError("", "Ten użytkownik został już usunięty.");
                }
                else
                {
                    ModelState.AddModelError("", "Ten użytkownik został już zmodyfikowany.");
                }
